...
R5 [4]: `pardosa-derive` is the blessed derive source for application
  types; substrate primitives may carry in-tree hand impls. The
  closed in-tree foreign-type `Encode`/`Decode` impl set is exactly
  `uuid::Uuid` (16 verbatim bytes, no length prefix), behind the
  `uuid` feature gate, under the orphan-rule split that places
  `Encode`/`Decode` in `pardosa-wire` and `Sealed`/`EventSafe` in
  `pardosa-schema`. No foreign impl admits a new `EventError`
  variant. Any new foreign-type entry requires a successor PGN ADR
  under PGN-0013's admission gate, naming the wire shape and the
  post-decode capacity-check surface. Every in-tree hand impl and
  every bounded-wrapper impl (PGN-0013) ships paired with two
  coverage entries: a `proptest` round-trip identity case
  (`decode(encode(t)) == t`) and a `cargo-fuzz` target exercising
  `decode` against arbitrary byte slices for zero panics.
