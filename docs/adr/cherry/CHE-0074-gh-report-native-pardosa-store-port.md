...
R6 [5]: The P3-i port keeps `RepoPresence::Removed` as a native event value. Removed tombstones continue to drive the existing delete-from-projection behaviour until the P3-ii CHE-0073 amendment replaces tombstones with Detach.
...
risks/migration: this is a hard cut for gh-report's event log. The P3-i checkpoint keeps Removed semantics unchanged; P3-ii separately amends CHE-0073 and changes removal from a native event value to a pardosa Detach transition.
