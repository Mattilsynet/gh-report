...
risks/migration: this is a hard cut for gh-report's event log. Removal is a pardosa Detach transition carried by the envelope `detached` flag, not a projection-side tombstone delete; rollback is by reverting the native-port commit range.
