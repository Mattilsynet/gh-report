...
R5 [5]: Domain-key→`AggregateId` resolution is held in `AppState` as three `DashMap<DomainKey, AggregateId>` indices (`run_index: DashMap<Uuid, AggregateId>`, `repo_index: DashMap<RepoIdentity, AggregateId>`, `delivery_index: DashMap<String, AggregateId>`), populated lazily on first reference and consulted by ApplicationServices before issuing `EventStore::load`
