...
R13 [5]: cherry-pit-storage OWNS the run-fencing invariant, scoped to one host's local filesystem and carrying no cross-instance authority (PGN-0016:R3): a run lock is created `O_EXCL` in the target directory, holds `LockMetadata` (run id, pid, hostname, created_at), is reclaimable only once `created_at` exceeds `DEFAULT_LOCK_TTL` or by explicit force-unlock, and releases on drop
