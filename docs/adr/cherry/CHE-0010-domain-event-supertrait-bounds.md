...
`DomainEvent` is the marker trait for all events. Its supertrait bounds constrain every event type in every cherry-pit system. Events fan out to multiple consumers, requiring `Clone`. Events cross thread boundaries in async runtimes, requiring `Send + Sync + 'static`. CHE-0074 retired the opaque-byte pardosa adapter (formerly CHE-0071): gh-report now persists through a native pardosa store port that does not round-trip `DomainEvent` through serde. The live infrastructure consumer for `Serialize + DeserializeOwned` is `cherry-pit-gateway`'s `rmp-serde` envelope encoding (CHE-0045), which keeps the bound load-bearing on the public trait. `Debug` and `PartialEq` stay excluded; users add them per type as needed.
...
| `Serialize` | `cherry-pit-gateway` envelope encoding through rmp-serde (CHE-0045) |
| `DeserializeOwned` | `cherry-pit-gateway` envelope replay through rmp-serde (CHE-0045) |
...
R1 [4]: DomainEvent requires Clone + Send + Sync + 'static +
  ...
  the serde pair is load-bearing for cherry-pit-gateway's rmp-serde
  envelope encoding (CHE-0045)
...
- The trait does not require native pardosa `GenomeSafe`; CHE-0074's native store port keeps genome-safety at the pardosa boundary and leaves domain events on the serde model.
