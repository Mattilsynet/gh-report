...
R3 [5]: composition uses explicit struct construction `App::new(gateway, store, bus, projections, dead_letter)` mirroring CHE-0049's `AppState::new` minimalism — no type-state builder, no fluent builder returning `Result<App, WiringError>`, no constructor macro; the `App<G, S, B, P, D>` type-parameter list expresses the wired graph shape directly and wiring completeness is enforced at compile time by parameter inference rather than by accumulator state; the fifth slot (`dead_letter: D`) is required by R7 + CHE-0024:R5 + CHE-0040:R3's dead-letter routing contract
