...
R5 [5]: multi-projection composition is a heterogeneous tuple `(D1, D2, …, Dn)` of `ProjectionDriver` instances with fixed arity ~8, parameterised on the App's storage adapter, satisfying CHE-0048:R6's deferral by giving compile-time wiring completeness via type inference; the missing `apply_one` per-event primitive is added by `ProjectionDriverExt`, declared inside cherry-pit-agent and importing `ProjectionDriver` from cherry-pit-projection without editing it (CHE-0048 stays untouched per FOCUS.md §8)
