...
**Rescoped 2026-08-27.** As written, this amendment's claim that "the library now defaults Origin validation to Strict" was true only of `build_projection_router`. It was false of `serve::build_router`, which hardcoded `AllowAbsent` at its `/ws` upgrade — so the crate-wide present-tense reading did not hold, and in practice no origin-strict WebSocket ran anywhere in production (U1). The claim is now crate-wide and true: every cherry-pit-web surface mounting a WS upgrade takes a `WsPolicy` (SEC-0012:R1) whose `WsPolicy::new` yields `Strict` without the caller naming it, and `serve` takes that default. `WsAuthLimits` is superseded by `WsPolicy`, which additionally carries `max_connections` — see SEC-0012 Consequences for why the two were fused, and CHE-0062:R2 for `LayerLimits` shedding `max_ws_connections`. The projection parameter named `ws_auth: WsAuthLimits` above reads `ws_policy: WsPolicy` after this rescope.

## Amendment 2026-08-27 (SEC-0012 carrier fusion)

`serve::build_router` and `serve::start` adopt the uniform grammar: `(state, limits: LayerLimits, ws_policy: WsPolicy, options: &ServeOptions, extra_routes)`. The three SEC-0003 numbers previously read off `serve`'s `ValidatedConfig` (`concurrency_limit`, `max_request_body_bytes`, `ws_max_connections`) migrate onto `LayerLimits`/`WsPolicy`; `ValidatedConfig` is renamed `ServeOptions` and shrinks to `csp` plus `error_page_key`, holding a pre-validated `HeaderValue` so the router builder carries no CSP panic path. `build_projection_router<P>` takes `ws_policy: WsPolicy` in place of `ws_auth: WsAuthLimits` and sources its WS cap from it rather than from `LayerLimits`. The CQRS `build_router` is unchanged in arity and takes no `WsPolicy` — it mounts no WS upgrade (CHE-0049:R3). Semver-major across cherry-pit-web, cherry-pit-app, and gh-report; precedent per Amendment 2026-05-16 Part 2.
