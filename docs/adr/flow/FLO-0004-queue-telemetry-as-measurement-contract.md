...
Tier: C
Status: Accepted
Parent-cross-domain: PGN-0015 — queue-telemetry-as-measurement-contract is the FLO-tier expression of PGN-0015's re-rescued directive that observability cardinality is bounded, by naming the queue-specific schema every async-infrastructure component must implement to make Reinertsen's queueing chapter actionable

## Related

References: PGN-0015, COM-0019
...
congestion), W23 (visible WIP). PGN-0015 governs cardinality budgets but does
...
Little's Formula when per-item tracing exceeds PGN-0015's cardinality budget;
...
R2 [8]: Wait-time estimates are computed from Little's Formula — queue-size
  ...
  tracing exceeds the cardinality budget set by PGN-0015.
...
risks/migration: poorly-bounded label cardinality can blow PGN-0015's budget —
