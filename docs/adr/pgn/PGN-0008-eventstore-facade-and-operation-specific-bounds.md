...
Sources rescue ADR-0018 (public `EventStore<T>` façade) and rescue ADR-0020 (`EventSafe` decoupled from codec traits — read against PGN-0003). `pardosa::store` is the sole adopter-facing module on the runtime crate; the historical root-level kit-of-parts (`pardosa::reader`, `pardosa::writer`, `pardosa::event_log`) is demoted to `pub(crate)` under the PGN-0009 clean-break posture. The façade composes existing runtime primitives into one entry: open / append / per-fiber read / line tail / same-fiber causal walk. `FiberId` is dragline-local — domain identity and causality live in payload (PGN-0011 names the optional routing accelerator).
...
R1 [5]: `pardosa::store` is the sole adopter-facing module on the runtime
  crate; root-level `pardosa::reader`, `pardosa::writer`, `pardosa::event_log`
  are `pub(crate)` and not adopter-visible.
