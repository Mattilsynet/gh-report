...
| 0.5     | 2026-05-16 | **Phase 2 v2 exit-criterion amended** (Track 4 LOC gate): original `wc -l < 2500` measured proxy (newlines incl. comments + tests), not substance; replaced with production-LOC non-regression gate via `scripts/prod-loc` (syn AST walker, excludes `#[cfg(test)]` + `tests/`) vs Phase-2-v2 baseline 1007. Doctrine: non-regression going forward, not a tightening floor (Track-4.3-internal ≤ 927 not inherited). **Mechanical enforcement via CI**: `scripts/` un-ignored from `.gitignore`; `scripts/prod-loc/` + `scripts/track4-verify/` Rust tools and historical awk artefacts now version-controlled; `.github/workflows/ci.yml` job `track4-gates` runs the 13-criterion harness on every push (exit 1 on FAIL, verified locally). Substance grounded in CHE-0062 / CHE-0049-Amendment-Part-2 / SMI-1..SMI-5. **Phase-state update:** Phase 2 v2 Tracks 0 + 0.5 + 1 + 2 + 4 closed; Tracks 3 + 4.4 + 5 remaining. Track 4 internal closure: Track 4.0 (commits `db5ecbc` → `f87949a`) → Track 4.2.A (`7dd97e4` + `5284afe`) → CHE-0062 draft+ratification (`f8e779c` + `2580886`) → Track 4.2.B-i+ii (`dafe468` + `f634de9`) → Track 4.3 consolidation (this commit). Track 4.4 (validate.rs migration to cherry-pit-web) split out as separate sub-mission per moltke scope-decision A. User ratifications 2026-05-16: (A) HTTP/WS contract frozen, (I) production-LOC measurement convention, (B+D) `scripts/` un-ignored + CI gate wired. Amendment is explicit per v2 doctrine (no silent reinterpretation of v1 goalposts). Companion: `FOCUS.md` v0.6. |
| 0.6     | 2026-05-16 | **v0.5 amendment retracted; bespoke scripts + LOC gate removed.** Same-day audit of `scripts/prod-loc` (574 LOC) + `scripts/track4-verify` (711 LOC) found ~10 of 13 criteria duplicate existing CI jobs (`build` / `test` / `clippy` / `fmt`); unique value (LOC gate, SMI rg checks) collapses to a handful of inline CI shell steps; LOC gate itself is a proxy (substance lives in commit diffs + ADRs, not line counts). User verdict: *"the gain is too small and the cost of drift and clutter is real. remove"*. Actions: `git rm` of `scripts/prod-loc`, `scripts/track4-verify`, `scripts/citation-diff`, and the four historical awk files; `.gitignore` re-adds `scripts/`; CI job `track4-gates` deleted; FOCUS.md §3 row + §8 Track-4 verify block rewritten. Track 4 epic `adr-fmt-ysaa` remains CLOSED (retraction removes mechanical duplication, not exit-criteria substance). **Phase-state unchanged from v0.5:** Tracks 0 + 0.5 + 1 + 2 + 4 closed; Tracks 3 + 4.4 + 5 remaining. **Doctrine lesson:** building tools to enforce a gate is higher-order ceremony — the tool's existence makes the proxy feel like substance. Sibling failure: SM2 `doc-markdown-fix` (also retracted 2026-05-16, 100× over-correction). Both rounds reinforce: substance is invariant-shaped, not tool-shaped. Companion: `FOCUS.md` v0.7. |
