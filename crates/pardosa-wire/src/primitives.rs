...
impl Encode for i8 {
    fn encode(&self, out: &mut Vec<u8>) {
        #[expect(
            ...
            reason = "i8<->u8 reinterpret is the canonical two's-complement wire mapping per PGN-0003:R1; try_from would reject signed bytes valid on the wire and break round-trip identity (PGN-0003:R5)"
        )]
        out.push(*self as u8);
    }
}
