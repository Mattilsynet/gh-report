# ADR-013: Page-class resource limits for denial-of-service protection
...
- `config.rs` — `PageClass` enum, `DecodeOptions` struct
- genome.md §Deserialization Limits, §Safety Contract (DoS protection)
- serde-rs/serde#744 (OOM via crafted count fields)
