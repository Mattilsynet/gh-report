# ADR-018: Non-zero padding bytes are a hard error
...
produces `DeError::NonZeroPadding`. The backward offset check (ADR-011, check #6)
...
- `error.rs` — `DeError::NonZeroPadding { offset }`
- genome.md §Verification (check #10), §Safety Contract, §Alignment Rules
- ADR-011 (verification check catalog, check #10 and #16)
