# ADR-032: Canonical encoding contract
...
- ADR-004: type and attribute rejection
- ADR-018: non-zero padding as hard error
- ADR-021: breadth-first heap ordering
- ADR-024: NaN bit-pattern preservation
- ADR-033: GenomeOrd marker trait for map keys
...
1. **Deterministic map key ordering.** `BTreeMap` iterates in `Ord` order
   ...
   serialized in iteration order. The `GenomeOrd` marker trait (ADR-033) restricts
...
2. **Fixed field ordering.** Struct fields serialize in declaration order. Enum
   ...
   no default values (ADR-029).

3. **Deterministic heap layout.** Variable-length data is written to the heap in
   breadth-first order (ADR-021). All offsets are forward-pointing. Empty containers
   allocate a heap entry with count=0 (ADR-020).

4. **Non-deterministic types rejected.** `HashMap`, `HashSet`, `usize`, `isize` are
   rejected at compile time (ADR-004). Serde attributes that alter serialization
...
5. **Padding canonicalization.** All alignment padding bytes are `0x00`. Non-zero
   padding is a hard deserialization error (ADR-018).

6. **Float bit-pattern preservation.** NaN bit patterns are preserved exactly as
   written (ADR-024). No NaN canonicalization — the same NaN bit pattern in
...
- ADR-004 (type/attribute rejection)
- ADR-018 (non-zero padding = hard error)
- ADR-020 (empty containers allocate heap entries)
- ADR-021 (breadth-first heap ordering)
- ADR-024 (NaN bit-pattern preservation)
- ADR-029 (reject `#[serde(default)]`)
- ADR-033 (GenomeOrd marker trait)
- genome.md §Determinism, §Verification, §Safety Contract
