# ADR-011: Inline verification check catalog — 20 structural invariants on every decode
...
ADR-006 establishes the always-verify philosophy and `#![forbid(unsafe_code)]`.
...
- ADR-006 (always-verify philosophy, `forbid(unsafe_code)`)
- genome.md §Verification, §Safety Contract
- `error.rs` — `DeError`, `FileError` enum variants
