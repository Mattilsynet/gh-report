...
pub struct TeamRoster {
    ...
    /// RFC-3339 observation instant carried from the persisted capture,
    /// or recorded when the live member fetch returns successfully.
    /// `None` means no observation instant is available; historical missing
    /// instants are never backfilled. This is observational metadata, not
    /// a source-order witness or a derived age.
