...
**How it works:** The daemon folds the event log into an in-memory projection containing per-repository evidence keyed by inventory key. During collection, if a repository's `updated_at` timestamp matches the projection entry AND the baseline entry's evidence was collected within `BASELINE_MAX_AGE_SECS` (4h default) of the current run, `should_reuse` keeps the previous evidence without re-evaluating the repository via the GitHub API.
